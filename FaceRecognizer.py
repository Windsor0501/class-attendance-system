...
class FaceRecognizer(object):
    ...
    def load_database(self, face_dirPath='./resources/face_database'): # change directory here?
        self.personName_list = os.listdir(face_dirPath) # use os.listdir() instead of os.walk()
        personId_list = []
        ...
        self.personId_1d_array = np.array(personId_list) # indexes for each person
        self.bincount_1d_array = np.bincount(self.personId_1d_array) # number of images for each person
        self.person_quantity = len(self.personName_list)
        self.image_quantity = len(personId_list)        
        print('Total number of people is %d, total number of faces is %d' %(self.person_quantity, self.image_quantity))
        startTime = time.time()
        batch_size = 1
        imageData_list = []
        count = 0
        ...
        print('Loading %d face images consuming %.4f seconds' %(self.image_quantity, usedTime))
        
        self.make_bestThreshold() # calculate best threshold (invalid for one photo system)
        self.bestThreshold = 1.60 # manually set threshold here
        print('Real threshold for face recognition {} which is finally set'.format(self.bestThreshold))
        # input('Stop here to check threshold, enter to continue...') # stop here to check threshold 
    ...
    def make_bestThreshold(self):
        ...
        for fold_index, (train_1d_array, test_1d_array) in enumerate(k_fold.split(index_1d_array)):
            ...
            print('Calculation %d, use threshold %.2f to get the best accuracy %.4f' %(fold_index+1, bestThreshold, max_accuracy))
            bestThreshold_list.append(bestThreshold)
        self.bestThreshold = np.mean(bestThreshold_list)
        print('Best Threshold %.4f' %self.bestThreshold)
        usedTime = time.time() - startTime
        print('Consuming %.2f seconds' %(usedTime))
    ...
    def make_randomDataSet(self):
        ...
        selected_personId_1d_array = np.where(self.bincount_1d_array >= 2)[0] # one enrolled person must have at least two images
        ...
        for i, personId in enumerate(same_personId_1d_array):
            ...
            index_1, index_2 = np.random.choice(selected_index, 2, replace=False) # randomly choose two images for given personId
        ...
        usedTime = time.time() - startTime
        print('Generating dataset randomly, total number of faces is %d, consuming %.2f seconds' %(sample_quantity, usedTime))
    ...
    def get_personName_1(self, image_3d_array):
        ...
        feature_2d_array = self.face_vectorizer.get_feature_2d_array(image_4d_array) # use insightface to get embedding
